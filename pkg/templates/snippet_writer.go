...
func NewSnippetWriter(w io.Writer, left, right string, funcs map[string]interface{}) *SnippetWriter {
	sw := &SnippetWriter{
		w:       w,
		left:    left,
		right:   right,
...
// Example:
//
// sw := generator.NewSnippetWriter(outBuffer, "$", "$")
// sw.Do(`The public type name is: $.type$`, map[string]interface{}{"type": t})
// return sw.Error()
//
// Where:
